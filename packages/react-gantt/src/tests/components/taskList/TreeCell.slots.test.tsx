...
describe("<TreeCell /> slots", () => {
  it("renders the default expand button and toggles on click", () => {
    ...
    const { getByRole } = render(
      <TreeCell taskId={7} depth={0} isParent isExpanded={false} onToggleExpand={onToggleExpand}>
        Task name
      </TreeCell>,
    );
  ...
  it("renders a placeholder (no button) for leaf rows", () => {
    const { queryByRole } = render(
      <TreeCell taskId={1} depth={0} isParent={false} isExpanded={false} onToggleExpand={vi.fn()}>
        x
      </TreeCell>,
    );
