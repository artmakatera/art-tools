...
describe("schedule", () => {
  it("respects dependencies", () => {
    ...
    const current = toMap([
      { id: 1, name: "a", startDate: jan(10), endDate: jan(12) },
      { id: 2, name: "b", startDate: jan(10), endDate: jan(12) }, // violates FS
    ]);
    ...
    expect(changed.get(2)?.endDate).toEqual(jan(15)); // duration (2) preserved
  });
  ...
  describe("SS dependency", () => {
    ...
    it("does not pull a successor earlier when its predecessor moves earlier", () => {
      ...
      move(current, 11, jan(5), jan(5)); // predecessor moves earlier
    ...
    it("does not push a successor later when its predecessor moves later but still satisfies the dependency", () => {
      const current = toMap(baseTasks());
      move(current, 12, jan(20), jan(21)); // successor sits later (has slack)
      move(current, 11, jan(15), jan(15)); // predecessor moves later, but before the successor
    ...
    it("pushes a successor later when its predecessor moves later the start of the successor task", () => {
      const current = toMap(baseTasks());
      move(current, 11, jan(20), jan(20)); // predecessor moves past the successor's start

      const changed = scheduleDependents(current, graph, 11);

      expect(changed.get(12)?.startDate).toEqual(jan(20)); // SS → aligns to predecessor start
      expect(changed.get(12)?.endDate).toEqual(jan(21)); // duration (1) preserved
    });

    it("push successor to start of predecessor when successor starts moves before predecessor", () => {
      const current = toMap(baseTasks());
      move(current, 12, jan(5), jan(6)); // successor dragged before its predecessor
      ...
      expect(changed.get(12)?.endDate).toEqual(jan(11)); // duration (1) preserved
    });
  });
  ...
  describe("SF dependency", () => {
    ...
    it("finishes the successor the day before the predecessor starts, with no lag, when the predecessor is after it", () => {
      const current = toMap(baseTasks());
      move(current, 11, jan(20), jan(20)); // predecessor moves well after the successor

      const changed = scheduleDependents(current, graph, 11);

      // SF → successor finishes adjacent to (the day before) the predecessor's start
      expect(changed.get(12)?.endDate).toEqual(jan(19));
    ...
    it("clamps a successor dragged before its anchor onto the predecessor's start", () => {
      const current = toMap(baseTasks());
      move(current, 12, jan(2), jan(3)); // successor dragged earlier than its anchor

      const changed = scheduleDependents(current, graph, 12);

      expect(changed.get(12)?.endDate).toEqual(jan(9)); // day before the predecessor's start
  ...
  describe("FS dependency", () => {
    ...
    it("pushes a successor to the day after the predecessor finishes later", () => {
      const current = toMap(baseTasks());
      move(current, 11, jan(18), jan(20)); // predecessor finishes later
    ...
    it("does not push a successor later when its predecessor moves later but still satisfies the dependency", () => {
      const current = toMap(baseTasks());
      move(current, 12, jan(25), jan(26)); // successor sits later (has slack)
      move(current, 11, jan(15), jan(15)); // predecessor finishes later, but before the successor
    ...
    it("clamps a successor dragged before the predecessor finishes onto the day after", () => {
      const current = toMap(baseTasks());
      move(current, 12, jan(2), jan(3)); // successor dragged before the predecessor finishes
  ...
  describe("FF dependency", () => {
    ...
    it("aligns the successor's finish with the predecessor's when it finishes later", () => {
      ...
      expect(changed.get(12)?.endDate).toEqual(jan(20)); // finishes with the predecessor
      expect(changed.get(12)?.startDate).toEqual(jan(19)); // duration (1) preserved
    });

    it("does not pull a successor earlier when its predecessor moves earlier", () => {
      const current = toMap(baseTasks());
      move(current, 11, jan(5), jan(5)); // predecessor finishes earlier
    ...
    it("clamps a successor dragged to finish before the predecessor onto its finish", () => {
      const current = toMap(baseTasks());
      move(current, 12, jan(2), jan(3)); // successor dragged to finish before the predecessor

      const changed = scheduleDependents(current, graph, 12);

      expect(changed.get(12)?.endDate).toEqual(jan(10)); // finishes with the predecessor
