...
describe("addWorkingMs — anchor projection is intentionally lossy", () => {
  ...
  describe("the round-trip rule", () => {
    ...
    const cases = [
      ...
      {
        name: "range start (working, forward-anchored only)",
        at: jan(5, 8),
        forward: true,
        backward: false,
      },
      {
        name: "range end (working, backward-anchored only)",
        at: jan(5, 17),
        forward: false,
        backward: true,
      },
      {
        name: "after a lunch gap (forward-anchored only)",
        at: jan(5, 13),
        forward: true,
        backward: false,
      },
