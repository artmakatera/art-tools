...
**Decision.** `readOnly?: boolean` on the chart (and on `GanttProvider`) removes every *user*
...
later without changing this prop). Gating on callback *presence* (`onTasksChange` absent ⇒
