...
**Decision.** Non-working time remains on the axis, greyed. dhtmlx's `skip_off_time` — which
...
uniform axis; compression would require replacing both with a working-time index. dhtmlx gates
this behind their PRO tier, which is a fair signal of its cost.
