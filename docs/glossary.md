...
| **Tooltip**               | The hover-only overlay on a bar. A slot with no default; the native `title` is the zero-config baseline and is suppressed once the slot is set (ADR-022).         |
| **Anchor name**           | The per-bar CSS dashed-ident the tooltip positions against. Set on the bar's row so bar and tooltip inherit one value; absent when no tooltip slot is configured. |
