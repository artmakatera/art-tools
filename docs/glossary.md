...
| **Hour range** | `"H:MM-H:MM"`, minute precision. Gaps *between* ranges in a day are non-working (that is how lunch is expressed). |
...
| **Walk** | Moving N units of *working* time from an anchor, skipping non-working time. Not addition — see ADR-007. |
...
| **Snapping** | Moving a *library-authored* date onto working time. Governed by `snapToWorking`. Never applied to consumer input (ADR-012). |
